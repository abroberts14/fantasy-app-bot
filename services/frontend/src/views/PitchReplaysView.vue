<template>
  <Card class="card-settings">
    ...
    <template #content class="content-flex">
      <div class="settings-item mt-4 mb-4">
        <FloatLabel>
          <AutoComplete
    ...
    <template #footer>
      <div class="footer-buttons">
        ...
        <Checkbox
          :disabled="isPageLoading"
          v-model="paOnly"
          inputId="plateAppearanceOnly"
          name="plateAppearanceOnly"
          binary
  ...
  <section class="video-section">
    <div v-if="currentPitches.length > 0">
      <MediaPlayer
        :videos="currentPitches"
        :reset="videoPlayerLoading"
...
export default defineComponent({
  ...
  setup() {
    ...
    function fetchValidDates(playerId, startDate, endDate) {
      ...
      axios
        .get(`/baseball/get-player-dates/${playerId}?${params}`)
        .then((response) => {
          if (response.data) {
    ...
    async function fetchPitches() {
      ...
      try {
        ...
        currentQuery.value = {
          ...
          date: formattedCalendarValue.value,
          name: selectedPlayer.value.name,
          current_pitch: currentPitch.value
        }
  ...
  mounted() {
    ...
    if (this.$route.query.playerId) {
      ...
      this.currentQuery = {
        player_id: this.$route.query.playerId,
        date: this.$route.query.date,
        name: decodeURIComponent(this.$route.query.name),
        current_pitch: parseInt(this.$route.query.current_pitch) || 0
      }
