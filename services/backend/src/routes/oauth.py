...
async def require_oauth_token(current_user: UserOutSchema = Depends(get_current_user)):
    oauth_token = await OAuthTokens.filter(user=current_user.id).first()
    if not oauth_token:
        raise HTTPException(
            status_code=403, detail="OAuth token required to perform this operation"
        )
    return oauth_token
...
async def get_oauth_tokens(
    ...
    user_id: int = Query(),  # Optional query parameter
):
    try:
        ...
        raise HTTPException(
            status_code=404, detail="OAuth token not found for the current user"
        )
...
async def get_oauth_token_by_id(user_id: int):
    try:
        ...
        raise HTTPException(
            status_code=404,
            detail="OAuth token not found for the specified ID or not belonging to the current user",
        )


@router.delete("/oauth/yahoo/tokens/{token_id}")
async def delete_oauth_token(
    token_id: int, current_user: UserOutSchema = Depends(get_current_user)
):
    try:
        ...
        raise HTTPException(
            status_code=404,
            detail="OAuth token not found or does not belong to the current user",
        )
